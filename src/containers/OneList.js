  ...
  <div className=" col-6 col-lg-3 col-md-6 p-1" key={id}>
    <div className="card">
      ...
      <div className="card-body bg-light">
        ...
        <Link to={`/cars/${id}`} className="btn btn-primary" key={id}>see details</Link>
      </div>
    </div>
  </div>;
        ...
        <div className="row">
          {
                cars.map((car) => {
                  ...
                  return (
                    <div key={keyId}>
                      <p key={keyId}>{color}</p>
                      <p key={keyId}>{engine}</p>
                      <Link to={`/cars/${id}`} className="btn btn-primary" key={keyId}>see details</Link>
                    </div>
                  );
                })
          }
